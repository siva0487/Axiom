...
namespace Retailinsights\ImportExport\Model\Source\Type;
...
class Ftp extends AbstractType
{
     ...
     *
     * @return \Retailinsights\ImportExport\Model\Filesystem\Io\Ftp
     * @throws \Magento\Framework\Exception\LocalizedException
     */
        ...
        if (!$this->getClient()) {
            ...
            if ($this->getData('ftp_host') && $this->getData('ftp_port') && $this->getData('ftp_user') && $this->getData('ftp_password')) {
                ...
                $settings = $this->_scopeConfig->getValue(
                    'Retailinsights_importexport/ftp',
            ...
            try {
                $connection = new \Retailinsights\ImportExport\Model\Filesystem\Io\Ftp();
                $connection->open(
                    $settings
                );
